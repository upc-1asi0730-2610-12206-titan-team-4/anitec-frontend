...
const visibleAnimals = computed(() => {
  if (iam.currentRole === 'rancher') return store.getAnimalsByOwnerId(iam.currentUserId);
  if (iam.currentRole === 'veterinarian') return store.getAnimalsByVeterinarianId(iam.currentUserId);
...
<template>
  <div class="panel">
    <div class="panel-header">
      ...
      <pv-button :label="t('animals.new')" icon="pi pi-plus" @click="router.push({name: 'livestock-animal-new'})"/>
    </div>
    ...
    <div class="animal-searchbar">
      ...
      <small>{{ filteredAnimals.length }} de {{ visibleAnimals.length }} animales</small>
    </div>

    <div v-if="!loaded" class="empty-state">Cargando animales...</div>

    <div v-else class="animal-directory-grid">
      <article v-for="animal in filteredAnimals" :key="animal.id" class="animal-directory-card">
        <header>
          <div>
        ...
        <dl>
          ...
          <div><dt>{{ t('animals.herd') }}</dt><dd>{{ store.getHerdName(animal.herdId) }}</dd></div>
        </dl>

        <footer>
          <pv-button :label="t('common.edit')" icon="pi pi-pencil" outlined @click="router.push({name:'livestock-animal-edit', params:{id: animal.id}})"/>
          <pv-button :label="t('common.delete')" icon="pi pi-trash" severity="danger" text @click="confirmDelete(animal)"/>
        </footer>
      </article>
    </div>

    <p v-if="loaded && !filteredAnimals.length" class="empty-state">No se encontraron animales con esa busqueda.</p>
