 ...
 */
const confirmDelete = (event) => confirm.require({
...
<template>
  <div class="panel">
    <div class="panel-header">
      <div><span class="section-chip">Sanitary BC</span><h2>{{ t('health.title') }}</h2></div>
      <pv-button :label="t('health.new')" icon="pi pi-plus" @click="router.push({name:'sanitary-health-event-new'})"/>
    </div>
    <div v-if="!loaded" class="empty-state">Cargando eventos sanitarios...</div>

    <div v-else class="record-card-grid health-card-grid">
      <article v-for="event in visibleHealthEvents" :key="event.id" class="record-card health-record-card">
        <header>
          <div>
        ...
        <dl>
          <div><dt>{{ t('health.animal') }}</dt><dd>{{ animalName(event.animalId) }}</dd></div>
          <div><dt>{{ t('health.type') }}</dt><dd>{{ event.type }}</dd></div>
          <div><dt>{{ t('health.date') }}</dt><dd>{{ event.date }}</dd></div>
          <div><dt>{{ t('health.veterinarian') }}</dt><dd>{{ textOrDefault(event.veterinarian, 'Sin asignar') }}</dd></div>
          <div><dt>{{ t('health.diagnosis') }}</dt><dd>{{ textOrDefault(event.diagnosis, 'Sin diagnostico') }}</dd></div>
          <div><dt>{{ t('health.treatment') }}</dt><dd>{{ textOrDefault(event.treatment, 'Sin tratamiento') }}</dd></div>
          <div><dt>{{ t('health.prescription') }}</dt><dd>{{ textOrDefault(event.prescription, 'Sin receta') }}</dd></div>
          <div><dt>{{ t('health.followUp') }}</dt><dd>{{ textOrDefault(event.followUp, 'Sin seguimiento') }}</dd></div>
          <div><dt>{{ t('health.nextDueDate') }}</dt><dd>{{ textOrDefault(event.nextDueDate, 'Sin fecha') }}</dd></div>
        </dl>

        <footer>
          <pv-button :label="t('common.edit')" icon="pi pi-pencil" outlined @click="router.push({name:'sanitary-health-event-edit', params:{id: event.id}})"/>
          <pv-button :label="t('common.delete')" icon="pi pi-trash" severity="danger" text @click="confirmDelete(event)"/>
        </footer>
      </article>
    </div>

    <p v-if="loaded && !visibleHealthEvents.length" class="empty-state">No hay eventos sanitarios registrados.</p>
